...
class Application_Plugin_SecureRequest extends Zend_Controller_Plugin_Abstract
{
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {
        ...
        $securePages = $this->_getSecurePages();

        switch(true)
        {
            case (in_array($resource, $securePages) || in_array($route, $securePages)) && !$request->isSecure() :
                $scheme = Zend_Controller_Request_Http::SCHEME_HTTPS;
                break;

            case $request->isSecure() && (!in_array($resource, $securePages) && !in_array($route, $securePages)):
